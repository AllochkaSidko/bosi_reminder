...
namespace BoSi_ReminderService
{
    class Program
    {
        static void Main()
        {
            ...
            const string serviceName = BoSi_ReminderWindowsService.CurrentServiceName;
            ...
            if (!serviceStarting)
            {
                if (isInstalled)
                {
                    DialogResult dr = MessageBox.Show(string.Format("Do You REALLY Want To Uninstall {0}", serviceName), "Danger", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            ...
            {
                var servicesToRun = new ServiceBase[] { new BoSi_ReminderWindowsService(), };
